...
- Конкретні профілі (`catalog`, `products`, `expenses`) — окремий таск після опису користувачем.
- Профілі описують: які поля брати, фільтри (IsFolder), зв'язки (залишки, ціни), режим (повний дамп / інкрементальний).
