...
- [ ] `dotnet run --project OneC.Cli -- test-connection` успішно підключається до бази Kplus
- [ ] Виводить версію платформи (`СистемнаяИнформация.ВерсияПриложения`) та конфігурації (`Метаданные.Версия`)
- [ ] При відсутності з'єднання — логується зрозуміла помилка (Serilog)
- [ ] Пароль у `appsettings.local.json` зашифрований (AES)
- [ ] Тести для `ConnectionStringProtector` (unit) проходять
