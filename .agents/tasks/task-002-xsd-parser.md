...
- [x] `list-catalogs` виводить список довідників (назва, поля, типи)
- [x] `list-enums` виводить переліки та їх значення
- [x] Парсер обробляє `CatalogObject.*`, `CatalogRef.*`, `EnumRef.*`, `CatalogTabularSectionRow.*`
- [x] Тести парсера (unit, на зразку XSD) проходять
